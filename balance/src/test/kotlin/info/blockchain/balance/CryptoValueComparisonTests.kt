...
class CryptoValueComparisonTests {
    ...
    fun `compare greater than false`() {
        val a = CryptoValue.etherFromWei(2)
        val b = CryptoValue.etherFromWei(3)
        (a > b) `should be` false
    }

    @Test
    fun `compare greater than false because equal`() {
        val a = CryptoValue.etherFromWei(3)
        val b = CryptoValue.etherFromWei(3)
        (a > b) `should be` false
    }
    ...
    fun `can't compare different currencies with greater than`() {
        ...
        { a > b } `should throw the Exception` Exception::class `with message` "Can't compare ETH and BTC"
    }

    @Test
    fun `can't compare different currencies with less than`() {
        val a = CryptoValue.bitcoinFromSatoshis(3)
        val b = CryptoValue.etherFromWei(3);
        { a < b } `should throw the Exception` Exception::class `with message` "Can't compare BTC and ETH"
    }
